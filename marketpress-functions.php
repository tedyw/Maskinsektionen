...
function _mp_cart_table($type = 'checkout', $echo = false) {
  ...
  if ($type == 'checkout-edit') {
    ...
    if ( $coupon = $mp->coupon_value($coupon_code, $total) ) {
      ...
      $content .=  '  <td class="mp_cart_subtotal_lbl" colspan="4">
            ...
            <div id="coupon-code" class="alignright" style="display: none;">
              ...
              <input type="submit" class="button" name="update_cart_submit" value="' . __('Apply &raquo;', 'mp') . '" />
            </div>
        </td>';
      $content .=  '</tr>';
    }
    ...
    $content .=  '  <td class="mp_cart_col_updatecart"><input type="submit" class="button" name="update_cart_submit" value="' . __('Update Cart &raquo;', 'mp') . '" /></td>';
    $content .=  '</tr></tfoot>';
...
function _mp_cart_login($echo = false) {
  ...
  if ( is_user_logged_in() || defined('MP_HIDE_LOGIN_OPTION') ) {
    ...
    $content .= '<br />';
    $content .= '<input type="submit" class="button" name="wp-submit" id="mp_login_submit" value="'.__('Login and Checkout &raquo;', 'mp').'" />';
...
function _mp_cart_shipping($editable = false, $echo = false) {
  ...
  if ($editable) {
    ...
    $content .= '<p class="mp_cart_direct_checkout">';
    $content .= '<input type="submit" class="button" name="mp_shipping_submit" id="mp_shipping_submit" value="'.__('Continue Checkout &raquo;', 'mp').'" />';
    $content .= '</p>';
    $content .= '</form>';
...
function _mp_cart_payment($type, $echo = false) {
  ...
  if ($type == 'form') {
    ...
    $content .= '<p class="mp_cart_direct_checkout">';
    $content .= '<input type="submit" "class="button" name="mp_payment_confirm" id="mp_payment_confirm" value="'.__('Confirm Payment &raquo;', 'mp').'" />';
    $content .= '</p>';
    $content .= '</form>';
  ...
  if (!empty($order_id)) {
    ...
    if ($order) { //valid order
      ...
      <form action="<?php mp_orderstatus_link(true, true); ?>" method="get">
    		...
    		<input type="submit" class="button" id="order-id-submit" value="<?php _e('Look Up &raquo;', 'mp'); ?>" />
      </form>
      <?php
    }
          ...
          <tr>
            <td class="mp_cart_login">
              <form name="loginform" id="loginform" action="<?php echo wp_login_url(); ?>" method="post">
                ...
                <br />
            		<input type="submit" class="button" name="wp-submit" id="mp_login_submit" value="<?php _e('Login &raquo;', 'mp'); ?>" />
...
function mp_list_products( $echo = true, $paginate = '', $page = '', $per_page = '', $order_by = '', $order = '', $category = '', $tag = '', $list_view = false ) {
  ...
  $content = '<ul id="mp_product_list" class="mp_'.$layout_type.' block-grid six-up mobile-two-up">';

  if ($last = $custom_query->post_count){

    $content .= get_products_html_grid($custom_query->posts);
...
function get_products_html_grid($post_array=array()){
  ...
  foreach ($post_array as $post) {
    ...
    $html .= '<li class="mp_one_tile '.implode($class, ' ').'">
...
function mp_product_price( $echo = true, $post_id = NULL, $label = true ) {
  ...
  $price = apply_filters( 'mp_product_price_tag', '<div class="mp_product_price">' . $label . $price . '</div>', $post_id, $label );
    ...
    if ($all_out) {
      ...
      if ($context == 'list') {
        if ($variation_select) {
          ...
          $button .= '<input class="mp_button_addcart button" type="submit" name="addcart" value="' . __('Add To Cart &raquo;', 'mp') . '" />';
        } else if ($mp->get_setting('list_button_type') == 'buynow') {
          $button .= '<input class="mp_button_buynow button" type="submit" name="buynow" value="' . __('Buy Now &raquo;', 'mp') . '" />';
        }
      } else {
...
function mp_product_image( $echo = true, $context = 'list', $post_id = NULL, $size = NULL ) {
  ...
  $image = get_the_post_thumbnail($post_id, $size, array('itemprop' => 'image', 'class' => 'centered mp_product_image_'.$context, 'title' => $title));
  
  if ( empty($image) && $context != 'single') {
    if ( !is_array($size) ) {
      $size = array( get_option($size."_size_w"), get_option($size."_size_h") );
    }
    $image = '<img width="'.$size[0].'" height="'.$size[1].'" itemprop="image" title="'.esc_attr($title).'" class="centered mp_product_image_'.$context.' wp-post-image" src="'.apply_filters('mp_default_product_img', $mp->plugin_url.'images/default-product.png').'">';
  }
...
function mp_products_filter() {
  ...
  ' <div class="mp_list_filter">
      <div class="row">
        <div class="filter-title four columns">'.__('Filter', 'mp').'</div>
          <form name="mp_product_list_refine" class="mp_product_list_refine" method="get">
              <div class="one_filter four columns">
